(function() {
  ...
  // ==================== PLAN MANAGEMENT MODAL (NEW MULTI-PLAN SYSTEM) ====================
  ...
  function renderActivePlansList(activePlans) {
    const container = document.getElementById('active-plans-list');
    if (!container) return;
    if (!activePlans || activePlans.length === 0) {
      container.innerHTML = '<em style="color: #888;">No active plans (free user)</em>';
      return;
    }
    container.innerHTML = activePlans.map(p => {
      const legacyTag = p.legacy ? ' <small style="opacity:0.7;">(legacy premium)</small>' : '';
      const pid = escapeHtml(p.legacy ? '__legacy__' : p.planId);
      return `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; background: rgba(255,215,0,0.08); border: 1px solid rgba(255,215,0,0.2); border-radius: 6px;">
          <div>
            <div style="font-weight: bold;">${escapeHtml(p.shortName || p.planId)}${legacyTag}</div>
            <small style="color: #888;">Expires ${fmtDate(p.expiresAt)}</small>
          </div>
          <button type="button" onclick="revokePlan('${pid}', '${escapeHtml(p.shortName || p.planId)}')" style="background: rgba(255,107,107,0.15); border-color: rgba(255,107,107,0.3); color: #ff6b6b;">🗑️ Revoke</button>
        </div>
      `;
    }).join('');
  }

  // Called from the inline EJS button. payload = { userId, username, activePlans }
  window.openSubscriptionModal = function(payload) {
    if (!payload || typeof payload !== 'object') {
      console.error('openSubscriptionModal expects an object payload');
      return;
    }
    window.__currentManageUserId = payload.userId;
    window.__currentManageUsername = payload.username;
    window.__currentActivePlans = payload.activePlans || [];

    const userEl = document.getElementById('sub-username');
    const grantUserIdEl = document.getElementById('grant-userId');
    const customExpiryEl = document.getElementById('grant-customExpiry');

    if (userEl) userEl.textContent = payload.username;
    if (grantUserIdEl) grantUserIdEl.value = payload.userId;
    if (customExpiryEl) customExpiryEl.value = '';

    renderActivePlansList(window.__currentActivePlans);
    openModal('subscription-modal');
  };

  window.revokePlan = async function(planId, planLabel) {
    const username = window.__currentManageUsername || 'this user';
    if (!confirm(`Revoke "${planLabel}" from ${username}?\n\nThis will remove their access to that plan immediately.`)) return;

    try {
      const response = await fetch('/admin/revoke-plan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: window.__currentManageUserId, planId })
      });
      const data = await response.json();
      if (data.success) {
        if (typeof showToast === 'function') showToast('✓ Plan revoked', 'success');
        // Refresh page so the table and ledger reflect the change
        setTimeout(() => location.reload(), 400);
      } else {
        alert('Failed to revoke plan: ' + (data.error || 'unknown error'));
      }
    } catch (err) {
      console.error('Revoke plan error:', err);
...
window.generateResetLink = async function(userId, username) {
  if (!confirm(`Generate a fresh password reset link for "${username}"?\n\nThis invalidates any previous link they had.`)) return;
  try {
    const res = await fetch('/admin/generate-reset-link', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId })
    });
    ...
    // Show modal-like prompt with copy button
    const url = data.resetUrl;
    const expires = new Date(data.expiresAt).toLocaleString();
    const wrap = document.createElement('div');
    wrap.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.75); z-index:9999; display:flex; align-items:center; justify-content:center; padding:20px;';
    wrap.innerHTML = `
      <div style="background:#1a1a2e; border:1px solid rgba(0,255,204,0.4); border-radius:12px; padding:24px; max-width:560px; width:100%; color:#fff;">
        <h3 style="margin:0 0 8px; color:#00ffcc;">🔗 Reset link for ${username}</h3>
        <p style="font-size:13px; color:#aaa; margin-bottom:14px;">Send this link to ${data.email} via WhatsApp / SMS. Valid until <strong>${expires}</strong>.</p>
        <textarea readonly id="reset-link-text" style="width:100%; height:80px; background:#0a0a0a; color:#00ffcc; border:1px solid rgba(255,255,255,0.2); border-radius:6px; padding:10px; font-family:monospace; font-size:12px; resize:none;">${url}</textarea>
        <div style="display:flex; gap:8px; margin-top:14px; justify-content:flex-end;">
          <button id="reset-copy-btn" style="padding:10px 16px; background:linear-gradient(135deg,#00ffcc,#00aaff); color:#000; border:none; border-radius:6px; font-weight:700; cursor:pointer;">📋 Copy Link</button>
          <button id="reset-close-btn" style="padding:10px 16px; background:rgba(255,255,255,0.1); color:#fff; border:1px solid rgba(255,255,255,0.2); border-radius:6px; cursor:pointer;">Close</button>
        </div>
      </div>`;
    document.body.appendChild(wrap);
    document.getElementById('reset-copy-btn').onclick = async () => {
      try {
        await navigator.clipboard.writeText(url);
        showToast('✓ Link copied to clipboard', 'success');
      } catch {
        document.getElementById('reset-link-text').select();
        document.execCommand('copy');
        showToast('✓ Link copied', 'success');
      }
    };
    document.getElementById('reset-close-btn').onclick = () => {
      wrap.remove();
      // Reload so the request still shows in the panel (it's still "pending" until user uses it)
    };
  } catch (err) {
...
window.adminVerifyEmail = async function(userId, username) {
  if (!confirm(`Manually mark "${username}" as email-verified?\n\nUse this only if you've confirmed their identity through another channel (e.g. WhatsApp).`)) return;
  try {
    const res = await fetch('/admin/verify-email-now', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId })
    });
    const data = await res.json();
    if (data.success) {
      showToast('✓ ' + username + ' is now verified', 'success');
...
window.adminResendVerification = async function(userId, username) {
  if (!confirm(`Generate a fresh verification code for "${username}"?\n\nWe'll show you the code so you can share it via WhatsApp.`)) return;
  try {
    const res = await fetch('/admin/resend-verification-code', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId })
    });
    ...
    wrap.style.cssText = 'position:fixed; inset:0; background:rgba(0,0,0,0.75); z-index:9999; display:flex; align-items:center; justify-content:center; padding:20px;';
    wrap.innerHTML = `
      <div style="background:#1a1a2e; border:1px solid rgba(0,255,204,0.4); border-radius:12px; padding:24px; max-width:420px; width:100%; color:#fff; text-align:center;">
        <h3 style="margin:0 0 8px; color:#00ffcc;">📧 New code for ${username}</h3>
        <p style="font-size:13px; color:#aaa; margin-bottom:18px;">Share this 6-digit code via WhatsApp to ${data.email}.</p>
        <div style="display:inline-block; padding:14px 28px; background:rgba(0,255,204,0.1); border:1px solid rgba(0,255,204,0.3); border-radius:10px; font-size:32px; font-weight:700; letter-spacing:8px; font-family:monospace; color:#00ffcc;">${data.code}</div>
        <div style="display:flex; gap:8px; margin-top:18px; justify-content:center;">
          <button id="vc-copy" style="padding:10px 16px; background:linear-gradient(135deg,#00ffcc,#00aaff); color:#000; border:none; border-radius:6px; font-weight:700; cursor:pointer;">📋 Copy Code</button>
          <button id="vc-close" style="padding:10px 16px; background:rgba(255,255,255,0.1); color:#fff; border:1px solid rgba(255,255,255,0.2); border-radius:6px; cursor:pointer;">Close</button>
        </div>
      </div>`;
    document.body.appendChild(wrap);
    document.getElementById('vc-copy').onclick = async () => {
      try { await navigator.clipboard.writeText(data.code); showToast('✓ Code copied', 'success'); } catch {}
    };
    document.getElementById('vc-close').onclick = () => { wrap.remove(); location.reload(); };
  } catch (err) {
    showToast('❌ Network error', 'error');
  }
};

window.dismissReset = async function(userId, username) {
  if (!confirm(`Dismiss the password reset request for "${username}"?\n\nTheir reset link will stop working immediately.`)) return;
  try {
    const res = await fetch('/admin/dismiss-reset', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId })
    });
